...
export default function ContactForm() {
  ...
  const validateField = (key: keyof ContactFormData, value: string) => {
    ...
    try {
      ...
      result = {
        success: false,
        error: err.issues?.[0]?.message ?? "Valor inválido",
      };
    }

    setErrors((prev) => ({
      ...prev,
      [key]: result.success ? undefined : result.error,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { submitContactForm } = await import("@/lib/submitContactForm");
      ...
      setDialog({
        title: "¡Éxito!",
        message:
          "Tu solicitud ha sido enviada. Nos pondremos en contacto pronto.",
        variant: "success",
      });
      setOpen(true);
  ...
  return (
    <>
      ...
      >
        ...
        <fieldset className="space-y-2">
          ...
          <RadioGroup
            ...
            onValueChange={(value) =>
              setField("contactPreference", value as "phone" | "email")
            }
          ...
          >
            <div className="flex items-center gap-2">
              <RadioGroupItem
                ...
                disabled={submitting}
              />
              <Label htmlFor="pref-phone" className="cursor-pointer">
                Teléfono/WhatsApp
              </Label>
            </div>
            <div className="flex items-center gap-2">
        ...
        <div>
          <div className="flex items-end justify-between mb-1">
            ...
            <span
              className={cn(
                "text-xs",
                msgLength > 1000 ? "text-red-600" : "text-muted-foreground"
              )}
            >
              {msgLength}/1000
            </span>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-800 max-w-sm">
            <DialogHeader>
                <DialogTitle className="text-green-600">{dialog.title}</DialogTitle>
